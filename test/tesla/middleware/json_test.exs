defmodule Tesla.Middleware.JsonTest do
  ...
  describe "Function: encode/2" do
    ...
    test "adds content-type header if one is not present and body is encodable" do
      env = %Tesla.Env{body: %{}}
    ...
    test "does not add content-type header if one is already present and body is encodable" do
      env = %Tesla.Env{body: %{}, headers: [{"content-type", "hello/world"}]}

      {:ok, %Tesla.Env{headers: headers}} = Tesla.Middleware.JSON.encode(env, [])

      assert [{"content-type", "hello/world"}] = headers
    end
  end
